...
@section('page_title', 'ເພີ່ມໂຄງການ')
...
<div class="max-w-3xl mx-auto">
  ...
  <form action="{{ route('admin.monk-programs.store') }}" method="POST" class="space-y-5">
    @csrf
    ...
    <div class="bg-surface-container-lowest rounded-xl shadow-[0px_2px_8px_rgba(26,28,29,0.06)] p-5">
      ...
      <div class="grid grid-cols-1 sm:grid-cols-2 gap-4 mb-4">
        <div>
          ...
          <input type="text" name="destination_country" value="{{ old('destination_country') }}" maxlength="100" required
                 ...
                 placeholder="ຕ.ຢ. ຈີນ, ໄທ, ຍີ່ປຸ່ນ">
        ...
        <div>
          ...
          <input type="number" name="year" value="{{ old('year', date('Y')) }}" required
                 ...
                 class="w-full rounded-lg border border-surface-container-high px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-primary/30 @error('year') border-red-400 @enderror">
          ...
          <div class="grid grid-cols-2 gap-2">
            @foreach($statuses as $key => $meta)
              <label class="flex items-center gap-2 px-2.5 py-1.5 rounded-lg border cursor-pointer transition-colors text-xs
                            {{ old('status', 'draft') === $key ? 'border-primary bg-primary/5' : 'border-surface-container-high hover:bg-surface-container-low' }}">
    ...
    <div class="bg-surface-container-lowest rounded-xl shadow-[0px_2px_8px_rgba(26,28,29,0.06)] p-5">
      ...
      <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
        <div>
          ...
          <input type="date" name="application_open" value="{{ old('application_open') }}"
                 class="w-full rounded-lg border border-surface-container-high px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-primary/30">
          @error('application_open')<p class="text-red-500 text-xs mt-1">{{ $message }}</p>@enderror
        </div>
        <div>
        ...
        <div>
          ...
          <input type="date" name="program_start" value="{{ old('program_start') }}"
                 class="w-full rounded-lg border border-surface-container-high px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-primary/30">
        ...
        <div>
          ...
          <input type="date" name="program_end" value="{{ old('program_end') }}"
                 class="w-full rounded-lg border border-surface-container-high px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-primary/30">
    ...
    <div class="bg-surface-container-lowest rounded-xl shadow-[0px_2px_8px_rgba(26,28,29,0.06)] p-5">
      ...
      <div class="grid grid-cols-2 gap-4 max-w-xs">
        <div>
          ...
          <input type="number" name="monks_quota" value="{{ old('monks_quota') }}"
                 min="1" max="9999"
                 class="w-full rounded-lg border border-surface-container-high px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-primary/30"
                 placeholder="ຈຳນວນ">
          @error('monks_quota')<p class="text-red-500 text-xs mt-1">{{ $message }}</p>@enderror
        </div>
        <div>
          <label class="block text-xs font-semibold mb-1">ຜ່ານການຄັດເລືອກ</label>
          <input type="number" name="monks_selected" value="{{ old('monks_selected', 0) }}"
                 min="0" max="9999"
                 class="w-full rounded-lg border border-surface-container-high px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-primary/30">
          @error('monks_selected')<p class="text-red-500 text-xs mt-1">{{ $message }}</p>@enderror
        </div>
      </div>
    </div>
    ...
    <div class="bg-surface-container-lowest rounded-xl shadow-[0px_2px_8px_rgba(26,28,29,0.06)] p-5">
      ...
      <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
        <div>
          ...
          <input type="url" name="application_url" value="{{ old('application_url') }}"
                 class="w-full rounded-lg border border-surface-container-high px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-primary/30"
                 placeholder="https://forms.google.com/...">
          @error('application_url')<p class="text-red-500 text-xs mt-1">{{ $message }}</p>@enderror
        </div>
        <div>
          <label class="block text-xs font-semibold mb-1">ອີເມວຕິດຕໍ່</label>
          <input type="email" name="contact_email" value="{{ old('contact_email') }}"
                 class="w-full rounded-lg border border-surface-container-high px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-primary/30"
                 placeholder="exchange@bfol.la">
