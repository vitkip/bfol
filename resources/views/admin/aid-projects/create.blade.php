...
@section('page_title', 'ເພີ່ມໂຄງການຊ່ວຍເຫຼືອ')
...
<div class="max-w-3xl mx-auto">
  ...
  <form action="{{ route('admin.aid-projects.store') }}" method="POST" class="space-y-5">
    @csrf
    ...
    <div class="bg-surface-container-lowest rounded-xl shadow-[0px_2px_8px_rgba(26,28,29,0.06)] p-5">
      ...
      <div class="grid grid-cols-1 sm:grid-cols-2 gap-4 mb-4">
        <div>
          ...
          <input type="text" name="country" value="{{ old('country') }}" maxlength="100" required
        ...
        <div class="flex flex-wrap gap-2">
          @foreach($types as $key => $meta)
            <label class="flex items-center gap-2 px-3 py-2 rounded-lg border cursor-pointer transition-colors
                          {{ old('type', 'religious') === $key ? 'border-primary bg-primary/5' : 'border-surface-container-high hover:bg-surface-container-low' }}">
              <input type="radio" name="type" value="{{ $key }}" class="accent-primary"
                     {{ old('type', 'religious') === $key ? 'checked' : '' }}>
      ...
      <div>
        ...
        <div class="flex flex-wrap gap-2">
          @foreach($statuses as $key => $meta)
            <label class="flex items-center gap-2 px-3 py-2 rounded-lg border cursor-pointer transition-colors
                          {{ old('status', 'planning') === $key ? 'border-primary bg-primary/5' : 'border-surface-container-high hover:bg-surface-container-low' }}">
              <input type="radio" name="status" value="{{ $key }}" class="accent-primary"
                     {{ old('status', 'planning') === $key ? 'checked' : '' }}>
              <span class="inline-flex items-center gap-1.5 text-xs font-semibold px-2 py-0.5 rounded-full {{ $meta['class'] }}">
                <i class="fas {{ $meta['icon'] }} text-[8px]"></i> {{ $meta['lo'] }}
              </span>
            </label>
      ...
      <div class="grid grid-cols-1 sm:grid-cols-3 gap-4">
        <div>
          ...
          <div class="relative">
            ...
            <input type="number" name="budget_usd" value="{{ old('budget_usd') }}"
          ...
          @error('budget_usd')<p class="text-red-500 text-xs mt-1">{{ $message }}</p>@enderror
        </div>
        <div>
    ...
    <div class="bg-surface-container-lowest rounded-xl shadow-[0px_2px_8px_rgba(26,28,29,0.06)] p-5">
      ...
      <input type="url" name="report_url" value="{{ old('report_url') }}"
             class="w-full rounded-lg border border-surface-container-high px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-primary/30"
             placeholder="https://drive.google.com/... ຫຼື URL ຂອງໄຟລ໌ລາຍງານ">
