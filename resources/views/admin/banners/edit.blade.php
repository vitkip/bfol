...
@section('page_title', 'ແກ້ໄຂ Banner')
...
<div class="max-w-3xl mx-auto">
  ...
  <div class="flex items-center gap-2 text-xs text-outline mb-4">
    ...
    <a href="{{ route('admin.banners.show', $banner) }}" class="hover:text-primary truncate max-w-[180px]">{{ $banner->title_lo }}</a>
    <i class="fas fa-chevron-right text-[10px]"></i>
    <span>ແກ້ໄຂ</span>
  </div>
  ...
  <form action="{{ route('admin.banners.update', $banner) }}" method="POST" enctype="multipart/form-data" class="space-y-5">
    ...
    <div class="bg-surface-container-lowest rounded-xl shadow-[0px_2px_8px_rgba(26,28,29,0.06)] p-5">
      ...
      <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
        <div>
          ...
          <input type="text" name="title_lo" value="{{ old('title_lo', $banner->title_lo) }}" maxlength="200" required
        ...
        <div>
          ...
          <input type="text" name="title_en" value="{{ old('title_en', $banner->title_en) }}" maxlength="200"
        ...
        <div>
          ...
          <input type="text" name="title_zh" value="{{ old('title_zh', $banner->title_zh) }}" maxlength="200"
                 class="w-full rounded-lg border border-surface-container-high px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-primary/30">
        </div>
      </div>
    </div>
      ...
      <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
        <div>
          ...
          <textarea name="subtitle_lo" rows="3"
                    class="w-full rounded-lg border border-surface-container-high px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-primary/30 resize-y">{{ old('subtitle_lo', $banner->subtitle_lo) }}</textarea>
        </div>
        <div>
          <label class="block text-xs font-semibold mb-1">EN</label>
          <textarea name="subtitle_en" rows="3"
                    class="w-full rounded-lg border border-surface-container-high px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-primary/30 resize-y">{{ old('subtitle_en', $banner->subtitle_en) }}</textarea>
        </div>
        <div>
          <label class="block text-xs font-semibold mb-1">ZH</label>
          <textarea name="subtitle_zh" rows="3"
                    class="w-full rounded-lg border border-surface-container-high px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-primary/30 resize-y">{{ old('subtitle_zh', $banner->subtitle_zh) }}</textarea>
        </div>
      </div>
    </div>
    ...
    {{-- ── ຮູບພາບ ── --}}
    <div class="bg-surface-container-lowest rounded-xl shadow-[0px_2px_8px_rgba(26,28,29,0.06)] p-5"
         x-data="{ replacing: false, preview: null }">
      <h3 class="font-bold text-sm text-on-surface mb-4 flex items-center gap-2">
      ...
      @if($banner->image_url)
        {{-- Current image --}}
        <div x-show="!replacing" class="space-y-3">
          <div class="flex items-start gap-3 p-3 bg-surface-container-low rounded-lg border border-surface-container-high">
            <img src="{{ $banner->image_url }}" alt="current"
                 class="w-24 h-16 object-cover rounded-lg border border-surface-container-high shrink-0">
            <div class="flex-1 min-w-0">
              <p class="text-xs font-semibold text-on-surface mb-1">ຮູບປັດຈຸບັນ</p>
              <p class="text-[11px] text-outline truncate">{{ $banner->image_url }}</p>
            </div>
          </div>
          <div class="flex gap-2">
            <button type="button" @click="replacing=true"
                    class="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-lg border border-surface-container-high text-xs font-semibold hover:bg-surface-container transition-colors">
              <i class="fas fa-exchange-alt text-[10px]"></i> ປ່ຽນຮູບ
            </button>
            <label class="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-lg border border-red-200 text-xs font-semibold text-red-600 hover:bg-red-50 transition-colors cursor-pointer">
              <input type="checkbox" name="clear_image" value="1" class="accent-red-500">
              ລຶບຮູບ
            </label>
          </div>
        </div>
        ...
        {{-- Replace panel --}}
        <div x-show="replacing" x-data="{ imgMode: 'file' }" style="display:none">
          <div class="flex gap-2 mb-3">
            <button type="button" @click="imgMode='file'"
              ...
              <i class="fas fa-upload text-[10px]"></i> ໄຟລ໌
            </button>
            ...
            <button type="button" @click="replacing=false"
                    class="ml-auto px-3 py-1.5 rounded-lg text-xs text-outline hover:bg-surface-container border border-surface-container-high transition-colors">
              ຍົກເລີກ
            </button>
          </div>
      ...
      @else
        {{-- No current image --}}
        <div x-data="{ imgMode: 'file', preview: null }">
          <div class="flex gap-2 mb-3">
            <button type="button" @click="imgMode='file'"
                    :class="imgMode==='file' ? 'primary-gradient text-white' : 'border border-surface-container-high hover:bg-surface-container'"
                    class="px-3 py-1.5 rounded-lg text-xs font-semibold transition-colors">
          ...
          <div x-show="imgMode==='url'">
            <input type="text" name="image_url" value="{{ old('image_url', $banner->image_url) }}" maxlength="500"
                   class="w-full rounded-lg border border-surface-container-high px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-primary/30"
                   placeholder="https://example.com/image.jpg">
          </div>
        </div>
    ...
    <div class="bg-surface-container-lowest rounded-xl shadow-[0px_2px_8px_rgba(26,28,29,0.06)] p-5">
      ...
      <div class="grid grid-cols-1 md:grid-cols-3 gap-4 mb-4">
        <div>
          ...
          <input type="text" name="btn_text_lo" value="{{ old('btn_text_lo', $banner->btn_text_lo) }}" maxlength="80"
                 class="w-full rounded-lg border border-surface-container-high px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-primary/30"
                 placeholder="ອ່ານເພີ່ມ">
        </div>
        <div>
          <label class="block text-xs font-semibold mb-1">ຊື່ປຸ່ມ (EN)</label>
          <input type="text" name="btn_text_en" value="{{ old('btn_text_en', $banner->btn_text_en) }}" maxlength="80"
                 class="w-full rounded-lg border border-surface-container-high px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-primary/30"
                 placeholder="Read more">
        </div>
        <div>
          <label class="block text-xs font-semibold mb-1">ຊື່ປຸ່ມ (ZH)</label>
          <input type="text" name="btn_text_zh" value="{{ old('btn_text_zh', $banner->btn_text_zh) }}" maxlength="80"
                 class="w-full rounded-lg border border-surface-container-high px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-primary/30"
                 placeholder="阅读更多">
        </div>
      </div>
      <div>
        ...
        <input type="text" name="btn_url" value="{{ old('btn_url', $banner->btn_url) }}" maxlength="500"
               class="w-full rounded-lg border border-surface-container-high px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-primary/30"
        ...
        <div class="flex flex-wrap gap-2">
          @foreach($styles as $key => $meta)
            <label class="flex items-center gap-2 px-3 py-2 rounded-lg border cursor-pointer transition-colors
                          {{ old('style', $banner->style) === $key ? 'border-primary bg-primary/5' : 'border-surface-container-high hover:bg-surface-container-low' }}">
              <input type="radio" name="style" value="{{ $key }}" class="accent-primary"
                     {{ old('style', $banner->style) === $key ? 'checked' : '' }}>
      ...
      <div class="mb-5">
        ...
        <div class="flex flex-wrap gap-2">
          @foreach($positions as $key => $meta)
            <label class="flex items-center gap-2 px-3 py-2 rounded-lg border cursor-pointer transition-colors
                          {{ old('position', $banner->position) === $key ? 'border-primary bg-primary/5' : 'border-surface-container-high hover:bg-surface-container-low' }}">
              <input type="radio" name="position" value="{{ $key }}" class="accent-primary"
                     {{ old('position', $banner->position) === $key ? 'checked' : '' }}>
