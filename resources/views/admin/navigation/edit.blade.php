...
<div class="max-w-2xl mx-auto">
  ...
  <form action="{{ route('admin.navigation.update', $navigation) }}" method="POST" class="space-y-5">
    @csrf
    @method('PUT')
    ...
    <div class="bg-surface-container-lowest rounded-xl shadow-[0px_2px_8px_rgba(26,28,29,0.06)] p-5">
      ...
      <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
        <div>
          ...
          <input type="text" name="label_lo" value="{{ old('label_lo', $navigation->label_lo) }}" required
        ...
        <div>
          ...
          <input type="text" name="label_en" value="{{ old('label_en', $navigation->label_en) }}"
        ...
        <div>
          ...
          <input type="text" name="label_zh" value="{{ old('label_zh', $navigation->label_zh ?? ''  ) }}"
                 class="w-full rounded-lg border border-surface-container-high px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-primary/30"
                 placeholder="菜单名称 (中文)">
        </div>
      </div>
    ...
    <div class="bg-surface-container-lowest rounded-xl shadow-[0px_2px_8px_rgba(26,28,29,0.06)] p-5">
      ...
      <div class="grid grid-cols-1 sm:grid-cols-2 gap-4 mb-3">
        <div>
          ...
          <input type="text" name="url" id="url-input" value="{{ old('url', $navigation->url) }}"
                 class="w-full rounded-lg border border-surface-container-high px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-primary/30"
        ...
        <div>
          ...
          <select name="target"
                  class="w-full rounded-lg border border-surface-container-high px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-primary/30">
            <option value="_self"  {{ old('target', $navigation->target) === '_self'  ? 'selected' : '' }}>ໜ້າດຽວກັນ (_self)</option>
            <option value="_blank" {{ old('target', $navigation->target) === '_blank' ? 'selected' : '' }}>ໜ້າໃໝ່ (_blank)</option>
          </select>
        </div>
      </div>
      ...
      <div>
        ...
        <div class="flex items-center gap-2">
          <i id="icon-preview" class="{{ $navigation->icon ?: 'fas fa-bars' }} text-blue-600 w-5 text-center"></i>
          <input type="text" name="icon" id="icon-input" value="{{ old('icon', $navigation->icon) }}"
      ...
      <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
        ...
        <div>
          ...
          <input type="number" name="sort_order" min="0" max="9999"
                 value="{{ old('sort_order', $navigation->sort_order) }}"
                 class="w-full rounded-lg border border-surface-container-high px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-primary/30">
    ...
    <div class="flex gap-3">
      <button type="submit"
        ...
        <i class="fas fa-save text-xs"></i> ບັນທຶກການແກ້ໄຂ
      </button>
...
<script>
  document.getElementById('icon-input').addEventListener('input', function() {
    ...
    preview.className = (this.value || 'fas fa-bars') + ' text-blue-600 w-5 text-center';
  });
