...
<div class="flex flex-col sm:flex-row sm:items-center justify-between gap-3 mb-5">
    <div>
        <h2 class="text-base font-bold text-on-surface">Categories</h2>
        <p class="text-xs text-outline mt-0.5">{{ $categories->count() }} ລາຍການ</p>
    </div>
    <a href="{{ route('admin.categories.create') }}"
        ...
        <i class="fas fa-plus text-xs"></i> ສ້າງໝວດໝູໃໝ່
    </a>
</div>
@if(session('success'))
            ...
            <tbody class="divide-y divide-surface-container-high">
                @forelse($categories as $category)
                    <tr class="hover:bg-surface-container-low transition-colors group">
                        <td class="px-4 sm:px-5 py-3">{{ $category->id }}</td>
                        <td class="px-4 py-3">{{ $category->name_lo }}</td>
                        <td class="px-4 py-3">{{ $category->name_en }}</td>
                        <td class="px-4 py-3">{{ $category->name_zh }}</td>
                        <td class="px-4 sm:px-5 py-3 text-right">
                            <a href="{{ route('admin.categories.show', $category) }}" class="inline-flex items-center gap-1 text-xs font-semibold text-info-700 hover:underline mr-2"><i class="fas fa-eye"></i>View</a>
                            <a href="{{ route('admin.categories.edit', $category) }}" class="inline-flex items-center gap-1 text-xs font-semibold text-warning-700 hover:underline mr-2"><i class="fas fa-edit"></i>Edit</a>
