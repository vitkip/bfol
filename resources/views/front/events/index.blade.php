...
@section('title', __('app.nav.events') . ' - ' . __('app.site_name'))
...
<section class="bg-midnight-950 py-16 md:py-24 relative overflow-hidden">
    ...
    <div class="container relative z-10 text-center">
        <h1 class="text-4xl md:text-5xl lg:text-6xl font-serif font-bold text-white mb-6">{{ __('app.nav.events') }}</h1>
        <p class="text-lg text-slate-300 max-w-2xl mx-auto">Explore upcoming and past events organized or supported by BFOL.</p>
    </div>
</section>
    ...
    <div class="container">
        ...
        <div class="flex flex-wrap items-center justify-center gap-3 mb-12">
            ...
            @foreach($categories as $cat)
                <a href="{{ route('front.events.index', ['category' => $cat->slug]) }}" 
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
                ...
                <div class="card group flex flex-col h-full bg-white rounded-2xl shadow-sm border border-slate-100 hover:shadow-xl hover:shadow-midnight-900/5 transition-all duration-300 overflow-hidden">
                    ...
                    <div class="p-6 flex flex-col flex-grow">
                        <div class="flex items-center gap-4 text-xs text-slate-400 font-medium uppercase tracking-wider mb-3">
                            <span class="flex items-center gap-1"><i class="far fa-clock text-gold-500"></i> {{ \Carbon\Carbon::parse($item->start_date)->format('d M Y') }}</span>
                            @if($item->location)
                                <span class="flex items-center gap-1 truncate"><i class="fas fa-map-marker-alt text-gold-500"></i> {{ $item->location }}</span>
                            @endif
                        </div>
                        ...
                        <h3 class="text-xl font-serif font-bold text-midnight-950 mb-3 line-clamp-2 group-hover:text-midnight-600 transition-colors">
                            <a href="{{ route('front.events.show', $item->slug) }}" class="focus:outline-none">
                        ...
                        <div class="mt-auto pt-4 border-t border-slate-100 flex items-center text-midnight-600 font-bold text-sm group-hover:text-gold-600 transition-colors">
                            View Event Details <i class="fas fa-arrow-right ml-2 group-hover:translate-x-1 transition-transform"></i>
                        </div>
                    </div>
            ...
            @empty
                <div class="col-span-full py-20 text-center">
                    ...
                    <h3 class="text-xl font-bold text-midnight-950 mb-2">No Events Found</h3>
                    <p class="text-slate-500">There are currently no events scheduled in this category.</p>
                </div>
            @endforelse
        </div>
